...
abstract class DLEFiles {
	...
	public static function init( $driver = null, $local_on_remote_errors = false, $root = null ) {
		...
		if ( !in_array($config['files_access'], array("public", "private") ) ) {
		...
		try {
			...
			self::$local = new Filesystem($adapter, ['directory_visibility' => $config['files_access'], 'visibility' => $config['files_access']]);
			self::$driver = 'local';
		...
		if( $driver ) {
			...
			try {
				...
				if( $driver == '1') {
					...
					$adapter = new League\Flysystem\AsyncAwsS3\AsyncAwsS3Adapter($client, $config['bucket_name'], $config['ftp_path'], new League\Flysystem\AsyncAwsS3\PortableVisibilityConverter( $config['files_access'] ) );
					...
					$client = new WebDAVClient([
						'baseUri' => trim($config['webdav_baseurl']),
						'userName' => trim($config['webdav_user']),
						'password' => trim($config['webdav_pass']),
					]);
				...
				self::$remote = new Filesystem($adapter, ['directory_visibility' => $config['files_access'], 'visibility' => $config['files_access']]);
				self::$driver = 'remote';

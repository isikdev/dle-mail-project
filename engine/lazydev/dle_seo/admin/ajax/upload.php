...
include_once ENGINE_DIR . '/lazydev/dle_seo/class/Upload.php';
...
$allowed_extensions = ['gif', 'jpg', 'png', 'jpeg', 'webp', 'jfif'];
$allowed_video = ['avi', 'mp4', 'wmv', 'mpg', 'flv', 'mp3', 'm4v', 'm4a', 'mov', '3gp', 'f4v', 'mkv'];
$allowed_files = explode(',', strtolower($user_group[$member_id['user_group']]['files_type']));

if (isset($_REQUEST['idRule'])) {
    $indexEdit = intval($_REQUEST['idRule']);
    if ($_REQUEST['subaction'] == 'deluploads') {
        $row = $db->super_query("SELECT og FROM " . PREFIX . "_dle_seo_news WHERE id='{$indexEdit}'");
        $og = xfieldsdataload($row['og']);
        if ($og['default']) {
            $img = explode('dle_seo/', $og['default'])[1];
            @unlink(ROOT_DIR . '/uploads/dle_seo/' . $img);
            unset($og['default']);
            $og = Helper::stringData($og);
            $db->query("UPDATE " . PREFIX . "_dle_seo_news SET og='{$og}' WHERE id='{$indexEdit}'");
        }
    } elseif ($_REQUEST['subaction'] == 'upload') {
        $FileCheck = new FileUploader();
        $get = $FileCheck->check_filename($_FILES['qqfile']['name']);

        if ($get) {
            $dir = ROOT_DIR . '/uploads/dle_seo/' . $get;
            if (move_uploaded_file($_FILES['qqfile']['tmp_name'], $dir)) {
                $imgInfo = @getimagesize(ROOT_DIR . '/uploads/dle_seo/' . $get);
                if (!in_array($imgInfo[2], [1, 2, 3, 18])) {
                    @unlink(ROOT_DIR . '/uploads/dle_seo/' . $get);
                    echo $FileCheck->msg_error($lang['upload_error_6'], 500);
                }

                $data_url = $config['http_home_url'] . 'uploads/dle_seo/' . $get;

                $link = $data_url;
                $flink = ", \"flink\":\"{$link}\"";

                $return_box = "<div class=\"uploadedfile\"><div class=\"info\">{$get}</div><div class=\"uploadimage\"><img style=\"width:auto;height:auto;max-width:100px;max-height:90px;\" src=\"" . $link . "\" /></div><div class=\"info\"><a href=\"#\" onclick=\"xfimagedelete('default_image','" . $get . "');return false;\">{$lang['xfield_xfid']}</a></div></div>";
                $xfvalue = $get;
                $xfvalue = addcslashes($xfvalue, "\t\n\r\"\\/");
                $return_box = addcslashes($return_box, "\t\n\r\"\\/");

                echo htmlspecialchars("{\"success\":true, \"returnbox\":\"{$return_box}\", \"xfvalue\":\"{$data_url}\", \"link\":\"{$link}\"{$flink}}", ENT_NOQUOTES, $config['charset']);
            } else {
                echo $this->msg_error($lang['images_uperr_3'], 403);
            }
        } else {
            echo $this->msg_error($lang['images_uperr_2'], 403);
        }
    }
    exit;
}

if (isset($_REQUEST['idRuleCat'])) {
    $catRule = Data::receive('cat');
    $indexEdit = intval($_REQUEST['idRuleCat']);
    $lastIndex = $indexEdit != -1 ? $indexEdit : count($catRule);

    if ($_REQUEST['subaction'] == "deluploads") {
        if ($catRule[$lastIndex]['default_image']) {
            @unlink(ROOT_DIR . '/uploads/dle_seo/cat/' . $catRule[$lastIndex]['default_image']);
        }
    } elseif ($_REQUEST['subaction'] == "upload") {
        $FileCheck = new FileUploader();
        $get = $FileCheck->check_filename($_FILES['qqfile']['name']);
        ...
        if ($get) {
            $dir = ROOT_DIR . '/uploads/dle_seo/cat/' . $get;
            if (move_uploaded_file($_FILES['qqfile']['tmp_name'], $dir)) {
                $imgInfo = @getimagesize(ROOT_DIR . '/uploads/dle_seo/cat/' . $get);
                if (!in_array($imgInfo[2], [1, 2, 3, 18])) {
                    @unlink(ROOT_DIR . '/uploads/dle_seo/cat/' . $get);
                    echo $FileCheck->msg_error($lang['upload_error_6'], 500);
                }

                $data_url = $config['http_home_url'] . 'uploads/dle_seo/cat/' . $get;
...
$seoId = 0;
if (isset($_REQUEST['seoId'])) {
    $seoId = $db->safesql(strip_tags(stripslashes($_REQUEST['seoId'])));
}
...
if ($_REQUEST['subaction'] == "upload") {
	...
	$uploader = new FileUploader($area, $seoId, $author, $t_size, $t_seite, $make_thumb, $make_watermark, $m_size, $m_seite, $make_medium);
...
if ($_POST['subaction'] == "deluploads") {
	...
	if (isset($_POST['images'])) {
		$row = $db->super_query("SELECT name FROM " . PREFIX . "_dle_seo_files WHERE type='0' AND author='$author' AND seoId='$seoId'");
		...
		foreach ($_POST['images'] as $image) {
			...
			foreach ($listimages as $dataimages) {
				if ($dataimages == $image) {
					...
					@unlink(ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . $image);
					@unlink(ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . 'thumbs/' . $image);
					@unlink(ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . 'medium/' . $image);
				}
		...
		$db->query("UPDATE " . PREFIX . "_dle_seo_files SET name='{$row['name']}' WHERE type='0' AND author='{$author}' AND seoId='{$seoId}'");
		if ($_POST['type'] == 'image') {
		    $getRow = $db->super_query("SELECT og FROM " . PREFIX . "_dle_seo WHERE id='{$seoId}'");
		    $og = xfieldsdataload($getRow['og']);
            $og['image'] = '';
            $arrTemp = [];
            foreach ($og as $xfielddataname => $xfielddatavalue) {
            ...
            $db->query("UPDATE " . PREFIX . "_dle_seo SET og='{$str}' WHERE id='{$seoId}'");
        }
	}
	...
	if ($user_group[$member_id['user_group']]['allow_file_upload'] AND is_array($_POST['files']) AND count($_POST['files'])) {
		foreach ($_POST['files'] as $file) {
			...
			$row = $db->super_query("SELECT id, onserver FROM " . PREFIX . "_dle_seo_files WHERE type='1' AND author='$author' AND seoId='$seoId' AND id='$file'");
			...
			if ($row['id']) {
				...
				$db->query("DELETE FROM " . PREFIX . "_dle_seo_files WHERE id='{$row['id']}'");
			}
		}
	}
}

$css_path = $config['http_home_url'] . 'engine/lazydev/dle_seo/admin/template/assets/frame.css';
...
$row = $db->super_query("SELECT name FROM " . PREFIX . "_dle_seo_files WHERE type='0' AND seoId='{$seoId}' AND author='{$author}'");
...
if ($row['name']) {
	...
	foreach ($listimages as $dataimages) {
		...
		if (file_exists(ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . $dataimages)) {

			$this_size = @filesize(ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . $dataimages);
			$img_info = @getimagesize( ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . $dataimages);
			$img_url = $config['http_home_url'] . 'uploads/dle_seo/' . $folder_prefix . $dataimages;

			if (file_exists(ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . 'medium/' . $dataimages)) {
				$img_url = $config['http_home_url'] . 'uploads/dle_seo/' . $folder_prefix . 'medium/' . $dataimages;
				$medium_data = 'yes';
			} else {
				$medium_data = 'no';
			}

			if (file_exists(ROOT_DIR . '/uploads/dle_seo/' . $folder_prefix . 'thumbs/' . $dataimages)) {
				$img_url = $config['http_home_url'] . 'uploads/dle_seo/' . $folder_prefix . 'thumbs/' . $dataimages;
				$thumb_data = 'yes';
			} else {
				$thumb_data = 'no';
			}
			...
			$data_url = $config['http_home_url'] . 'uploads/dle_seo/' . $folder_prefix . $dataimages;
...
$db->query("SELECT * FROM " . PREFIX . "_dle_seo_files WHERE type='1' AND author='$author' AND seoId='$seoId'");

while ($row = $db->get_row()) {
	if ($row['size']) {
		$this_size = formatsize($row['size']);
	} else {
		$this_size = formatsize(@filesize(ROOT_DIR . '/uploads/dle_seo/' . $row['onserver']));
	}
	...
	if(in_array($file_type, $allowed_video)) {
		if( $file_type == 'mp3') {
			...
			$data_url = $config['http_home_url'] . 'uploads/dle_seo/' . $row['onserver'];
			$file_play = 'audio';
		} else {
...
$im_show = $uploaded_list ? "tabClick(0);" : '';
...
jQuery(function($){
	...
	var uploader = new qq.FileUploader({
		...
		action: '{$root}engine/lazydev/dle_seo/admin/ajax/upload.php',
		maxConnections: 1,
		encoding: 'multipart',
        sizeLimit: {$max_file_size},
		allowedExtensions: ['{$simple_ext}'],
	    params: {"subaction" : "upload", "seoId" : "{$seoId}", "area" : "{$area}", "author" : "{$author}", "user_hash" : "{$dle_login_hash}"},
...
function upload_from_url( url ) {
	...
	$.post("{$root}engine/lazydev/dle_seo/admin/ajax/upload.php", {news_id: "{$seoId}", imageurl: copyurl, t_size: t_size, t_seite: t_seite, make_thumb: make_thumb, m_size: m_size, m_seite: m_seite, make_medium: make_medium, make_watermark: make_watermark, area: "{$area}", author: "{$author}", subaction: "upload", user_hash: "{$dle_login_hash}"}, function(data){
		if (data.success) {

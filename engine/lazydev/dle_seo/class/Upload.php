...
include_once (DLEPlugins::Check(ENGINE_DIR . '/lazydev/dle_seo/class/thumb.class.php'));
...
class FileUploader
{
	private $allowed_extensions = ['gif', 'jpg', 'jpeg', 'png', 'webp', 'jfif'];
	private $allowed_video = ['avi', 'mp4', 'wmv', 'mpg', 'flv', 'mp3', 'm4v', 'm4a', 'mov', '3gp', 'f4v', 'mkv'];
    ...
    function __construct ($area = '', $seoId = '', $author = '', $t_size = '', $t_seite = '', $make_thumb = true, $make_watermark = true, $m_size = 0, $m_seite = 0, $make_medium = false)
	{        
        ...
        $this->seoId = $db->safesql(strip_tags(stripslashes($seoId)));
        $this->t_size = $t_size;
	...
	function FileUpload()
	{
		...
		if (!is_dir(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX)) {
			@mkdir(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX, 0777);
			@chmod(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX, 0777);
			@mkdir(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs', 0777);
			@chmod(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs', 0777);
		}

		if (!is_dir(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'medium')) {
			@mkdir(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'medium', 0777);
			@chmod(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'medium', 0777);
		}
		...
		if (!is_dir(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX)) {
			return $this->msg_error($lang['upload_error_0'] . ' /uploads/dle_seo/' . FOLDER_PREFIX, 403);
		}

		if (!is_writable(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX)) {
			return $this->msg_error($lang['upload_error_1'] . ' /uploads/dle_seo/' . FOLDER_PREFIX . ' ' . $lang['upload_error_2'], 403);
		}

		if (!is_writable(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs')) {
			return $this->msg_error($lang['upload_error_1'] . ' /uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs/ ' . $lang['upload_error_2'], 403);
		}

		if (!is_writable(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'medium' ) ) {
			return $this->msg_error($lang['upload_error_1'] . ' /uploads/dle_seo/' . FOLDER_PREFIX . 'medium/ ' . $lang['upload_error_2'], 403);
		}
		...
		if ($config['files_allow'] AND $user_group[$member_id['user_group']]['allow_file_upload'] AND in_array($type, $this->allowed_files)) {
			...
			if ($user_group[$member_id['user_group']]['max_files']) {
				$row = $db->super_query("SELECT COUNT(*) as count FROM " . PREFIX . "_dle_seo_files WHERE type='1' AND author='{$this->author}' AND seoId='{$this->seoId}'");
			...
			$uploaded_filename = $this->file->saveFile(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX, $filename, $this->use_prefix);

			if ($uploaded_filename) {
				@chmod(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename, 0666);
				
				$added_time = date("Y-m-d H:i:s");
				$size = filesize(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename);

				$db->query("INSERT INTO " . PREFIX . "_dle_seo_files (seoId, name, onserver, author, date, size, type) VALUES 
				('{$this->seoId}', '{$filename}', '". FOLDER_PREFIX ."{$uploaded_filename}', '{$this->author}', '{$added_time}', '{$size}', '1')");
				$id = $db->insert_id();
				if (in_array($type, $this->allowed_video)) {
					if ($type == 'mp3') {
						$file_link = $config['http_home_url'] . 'engine/skins/images/mp3_file.png';
						$data_url = $config['http_home_url'] . 'uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename;
						$file_play = 'audio';
					} else {
			...
			if ($user_group[$member_id['user_group']]['max_images']) {
				$row = $db->super_query("SELECT name FROM " . PREFIX . "_dle_seo_files WHERE type='0' AND author='{$this->author}' AND seoId='{$this->seoId}'");
				if ($row['name']) {
			...
			if ($uploaded_filename) {
				...
				$i_info = @getimagesize(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename);
		
				if (!in_array($i_info[2], [1, 2, 3, 18])) {
					@unlink(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename);
					return $this->msg_error($lang['upload_error_6'], 500);
				}

				$thumb = new thumbnail(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename);
				
				$row = $db->super_query("SELECT COUNT(*) as count FROM " . PREFIX . "_dle_seo_files WHERE type='0' AND seoId='{$this->seoId}' AND author='{$this->author}'");
				if (!$row['count']) {
					$inserts = FOLDER_PREFIX . $uploaded_filename;
					$db->query( "INSERT INTO " . PREFIX . "_dle_seo_files (name, author, seoId, date) values ('{$inserts}', '{$this->author}', '{$this->seoId}', '{$added_time}')");
				} else {
					$row = $db->super_query("SELECT name FROM " . PREFIX . "_dle_seo_files WHERE type='0' AND seoId='{$this->seoId}' AND author='{$this->author}'");
					...
					if ($error_image != 'stop') {
						...
						if (dle_strlen($row['name'], $config['charset']) < 65000) {
							$db->query("UPDATE " . PREFIX . "_dle_seo_files SET name='{$row['name']}' WHERE type='0' AND seoId='{$this->seoId}' AND author='{$this->author}'");
						}
					}
				}
				...
				if ($this->make_thumb) {
					if ($thumb->size_auto($this->t_size, $this->t_seite)) {
						...
						$thumb->save(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs/' . $uploaded_filename);
						@chmod(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs/' . $uploaded_filename, 0666);
					}
				}
				...
				if ($this->make_medium) {
					...
					if ($thumb->size_auto($this->m_size, $this->m_seite)) {
						...
						$thumb->save(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'medium/' . $uploaded_filename);
						@chmod(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'medium/' . $uploaded_filename, 0666);
					}
				}
				...
				if ($member_id['user_group'] == 1) {
					...
					if ($this->make_watermark OR $config['max_up_side'] OR $re_save) {
						...
						if ($re_save) {
							$thumb->save(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename);
						}
					}
				} else {
					$thumb = new thumbnail(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename );
				...
				$img_url = $config['http_home_url'] . 'uploads/dle_seo/' . FOLDER_PREFIX . $uploaded_filename;

				if (file_exists(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'medium/' . $uploaded_filename)) {
					$img_url = $config['http_home_url'] . 'uploads/dle_seo/' . FOLDER_PREFIX . 'medium/' . $uploaded_filename;
					$medium_data = 'yes';
					$tm_url = $img_url;
				} else {
					$medium_data = 'no';
				...
				if (file_exists(ROOT_DIR . '/uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs/' . $uploaded_filename)) {
					$img_url = $config['http_home_url'] . 'uploads/dle_seo/' . FOLDER_PREFIX . 'thumbs/' . $uploaded_filename;
					$thumb_data = 'yes';
					$th_url = $img_url;
				} else {
					$thumb_data = 'no';

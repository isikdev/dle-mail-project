...
class Conditions
{
    ...
    static function realize($template, $row = false, $tag = 'if')
    {
        ...
        self::$xfields = xfieldsdataload($row['xfields']);
    ...
    static function conditionsMatching($condition)
    {
		...
		if (dle_strpos($conditionMatching[1], 'xfvalue', 'UTF-8') !== false) {
            ...
            $conditionMatching[1] = stripslashes(self::$xfields[$conditionMatching[1]]);
        } else {
        ...
        if (dle_strpos($conditionMatching[3], 'xfvalue', 'UTF-8') !== false) {
            ...
            $conditionMatching[3] = stripslashes(self::$xfields[$conditionMatching[3]]);
        } elseif (self::$row[$conditionMatching[3]]) {
            $conditionMatching[3] = self::$row[$conditionMatching[3]];
        }

<?php
/**
 * Конфиг и языковый файл
 *
 * @link https://lazydev.pro/
 * @author LazyDev <email@lazydev.pro>
 **/

namespace LazyDev\Subscribe;

class Data
{
    static private $data = [];

    /**
     * Загрузить конфиг и языковый пакет
     */
    static function load()
    {
		$path = realpath(__DIR__ . '/..');

		self::$data['config'] = include $path . '/data/config.php';

		if (isset(self::$data['config']['lang'])) {
			self::$data['config']['lang'] = in_array(self::$data['config']['lang'], ['ua', 'en', 'ru']) ? self::$data['config']['lang'] : 'ru';
		} else {
			self::$data['config']['lang'] = 'ru';
		}

		self::$data['lang'] = include $path . '/lang/lang_' . self::$data['config']['lang'] . '.lng';
    }

    /**
     * Вернуть массив данных
     *
     * @param   string  $key
     * @return  array
     */
    static function receive($key)
    {
        return self::$data[$key];
    }

    /**
     * Получить данные с массива по ключу
     *
     * @param    string|array   $key
     * @param    string         $type
     * @return   mixed
     */
    static public function get($key, $type)
    {
        if (is_array($key)) {
            return Helper::multiArray(self::$data[$type], $key, count($key));
        }

        return self::$data[$type][$key];
    }

}